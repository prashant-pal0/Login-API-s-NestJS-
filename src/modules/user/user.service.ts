...
export class UserService {
  ...
  async getUserProfile(userId: string) {
    try {
      const userDetails = await this.userRepository.findBy({ id: userId })
      if (!userDetails) throw new Error(`${userId} User not found`)
      return {message: "User detail fetch successfully", data: userDetails}
